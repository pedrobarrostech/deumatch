...
namespace DotNetCoreAppExample.Services.Api.Controllers
{
    public class AccountController : BaseController
    {
        ...
        private readonly IUsuariosAppService _usuarioDadosAppService;
        ...
        public AccountController(
                    ...
                    IUsuariosAppService usuarioDadosAppService)
        {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterViewModel model)
        {
            ...
            user.Claims.Add(new IdentityUserClaim<Guid> { ClaimType = "Contatos", ClaimValue = "Ver" });
            user.Claims.Add(new IdentityUserClaim<Guid> { ClaimType = "Contatos", ClaimValue = "Gerenciar" });
            user.Claims.Add(new IdentityUserClaim<Guid> { ClaimType = "Contatos", ClaimValue = "GerenciarTelefones" });

            var result = await _userManager.CreateAsync(user, model.Password);

            if (result.Succeeded)
            {
                var usuarioDados = new UsuarioViewModel
                {
                    Id = user.Id,
                    Nome = model.Nome,
                };
        ...
        private async Task<object> GerarTokenUsuario(LoginViewModel login)
        {
            ...
            var orgUser = _usuarioDadosAppService.FindById(user.Id);
